...
- **事件最早开始时间`Ve[i]`**：从源点$V_0$到定点$V_i$的最长路径长度。
  $$
  Ve[i] = \max\{Ve[i - 1] + dur(<V_{i - 1}, V_i>)\}
  $$

  > 一定要完成前置节点才能完成该节点。

- **事件最完开始时间`Vl[i]`**：保证汇点$V_{n - 1}$在`Ve[i - 1]`时刻完成的前提下，事件$V_i$的允许的最迟开始时间。
  $$
  Vl[i] = \min\{Vl[i + 1] - dur(<V_{i}, V_{i +1}>)\}
  $$

- **活动最早可能开始时间`e[k]`**：设活动$a_k$在边$<V_i, V_j>$上，则$e[k] = Ve[i]$

- **活动最迟允许开始时间`l[k]`**：$l[k] = Vl[j] - dur(<V_i, V_j>)$

- **时间余量`l[k]-e[k]`**：如果$l[k] - e[k] = 0$，则$a_k$是没有时间余量的关键活动。
